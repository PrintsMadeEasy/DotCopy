...
for($i=1; $i<185; $i++){
	...
	$dbCmd->Query("SELECT DISTINCT IPaddress FROM bannerlog USE INDEX (bannerlog_Date)
					WHERE DomainID = 1 AND Name LIKE '".$googleTrackingCode."' 
	...
	foreach($ipAddressArr as $thisIPaddress){
		$dbCmd->Query("SELECT COUNT(*) FROM bannerlog USE INDEX (bannerlog_Date) WHERE  DomainID = 1 AND Name LIKE '".$googleTrackingCode."' AND IPaddress='".$thisIPaddress."'
						AND (Date BETWEEN '" . DbCmd::FormatDBDateTime($start_timestamp) . "' AND '" . DbCmd::FormatDBDateTime($end_timestamp) . "') ");
	...
	foreach(array_keys($ipAddressWithMultiples) as $thisIPaddressMultiplied){
		$dbCmd->Query("SELECT COUNT(*) FROM orders  USE INDEX (orders_DateOrdered) 
						...
						AND (DateOrdered BETWEEN '" . DbCmd::FormatDBDateTime($start_timestamp) . "' AND '" . DbCmd::FormatDBDateTime($end_timestamp + 60*60*24*2) . "') ");
		$conversionsCount = $dbCmd->GetValue();
