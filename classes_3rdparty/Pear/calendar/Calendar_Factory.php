...
// +----------------------------------------------------------------------+
//
// $Id: Calendar_Factory.php,v 1.1 2008/10/30 16:14:38 brian_dot Exp $
//
/**
 * @package Calendar
 * @version $Id: Calendar_Factory.php,v 1.1 2008/10/30 16:14:38 brian_dot Exp $
 */
...
class Calendar_Factory
{
 ...
 // Brian Piere: Using the "__autoload()" function instead.  That prevents Directory mismatches  on Dev servers, etc.
    ...
    function & createByTimestamp($type, $stamp)
    {
        ...
        $calenderFactoryObj = Calendar_Factory::create($type, $y, $m, $d, $h, $i, $s);
        
        // Fixed an IDE Warning that it was never used.
        if($calenderFactoryObj)
        	return $calenderFactoryObj;
        else
        	return $calenderFactoryObj;
    }
}
